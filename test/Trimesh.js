...
module.exports = {
  ...
  updateTree: {
    scaled: function(test) {
      ...
      test.equal(
        bigMesh.aabb.upperBound.x,
        mesh.aabb.upperBound.x * 2,
        'AABB does not scale with the mesh!'
      );

      test.equal(
        bigMesh.tree.aabb.upperBound.x,
        mesh.tree.aabb.upperBound.x,
        'Octree AABB scales with the mesh, which is wrong!'
      );
  ...
  getVertex: {
    unscaled: function(test) {
      ...
      test.deepEqual(
        vertex,
        new Vec3(mesh.vertices[0], mesh.vertices[1], mesh.vertices[2])
      );
      test.done();
    },
    scaled: function(test) {
  ...
  getNormal: function(test) {
    ...
    test.deepEqual(
      new Vec3(mesh.normals[0], mesh.normals[1], mesh.normals[2]),
      normal
    );
    test.done();
  },
